...
def init_switch_(tensor: pt.Tensor, scale: float = 0.1) -> pt.Tensor:
    """
    ...
    Draw elements from a truncated normal distribution using std = sqrt(scale /
    fan_in). The authors recommend scale = 0.1. As the Switch Transformer is
    originally implemented in Mesh TensorFlow, we use the same relative bounds
    as TensorFlow: 2 standard deviations above and below the mean:
...
def init_palm_(tensor: pt.Tensor) -> pt.Tensor:
    """
    Initialization based on "PaLM: Scaling Language Modeling with Pathways"
    (Chowdhery et al. 2022, https://arxiv.org/abs/2204.02311). We replace the
    normal distribution with a truncated normal distribution.
