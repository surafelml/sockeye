...
def init_switch_(tensor: pt.Tensor, scale: float = 0.1, use_uniform: bool = False) -> pt.Tensor:
    ...
    Transformer is originally implemented in Mesh TensorFlow, we use the same
    relative bounds as TensorFlow: 2 standard deviations above and below the
    mean:
    ...
    :param use_uniform: Use uniform distribution instead of truncated normal
                        distribution, applying the same bounds.
...
def init_palm_(tensor: pt.Tensor) -> pt.Tensor:
    ...
    with Pathways" (2022, https://arxiv.org/abs/2204.02311)
